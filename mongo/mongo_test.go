...
func Instance() Dao {
	return &Imp{
		Url: gotato.GetGotatoInstance().Mongo("test").Url,
		DB:  gotato.GetGotatoInstance().Mongo("test").DB,
		c:   gotato.GetGotatoInstance().Mongo("test").c,
	}
}
...
func (i Imp) Insert(collection string, docs any) (err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).Insert(docs)
}
...
func (i *Imp) IsExistKey(collection string, bs bson.M) (bool, error) {
	...
	if err := s.DB(i.DB).C(collection).Find(bs).One(value); err != nil {
...
func (i *Imp) QueryData(collection string, bs bson.M, value any) (err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	err = s.DB(i.DB).C(collection).Find(bs).One(value)
	return
}
...
func (i *Imp) GetCount(collection string, bs bson.M) (count int, err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).Find(bs).Count()
}
func (i *Imp) QueryAllData(collection string, bs bson.M, value any) (err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).Find(bs).All(value)
}

func (i *Imp) QueryMultiData(collection string, bs bson.M, sort string, offset, limit int, value any) (int, error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	count, err := s.DB(i.DB).C(collection).Find(bs).Count()
	if err != nil {
		return 0, err
	}
	if err := s.DB(i.DB).C(collection).Find(bs).Sort(sort).Skip(offset).Limit(limit).All(value); err != nil {
		return 0, err
	}
	return count, nil
}
...
func (i *Imp) UpdateData(collection string, bs bson.M, value any) (err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).Update(bs, value)
}
func (i *Imp) UpdateAllData(collection string, bs bson.M, value any) (num int, err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	info, err := s.DB(i.DB).C(collection).UpdateAll(bs, value)
	if err != nil {
		return 0, err
	}
...
func (i *Imp) UpdateDataById(collection string, id, value any) (err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).UpdateId(id, value)
}
func (i *Imp) Upsert(collection string, bs bson.M, value any) (err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	_, err = s.DB(i.DB).C(collection).Upsert(bs, value)
	return err
}
...
func (i *Imp) DelDataById(collection string, id any) (err error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).RemoveId(id)

}
func (i *Imp) DelData(collection string, bs bson.M) error {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).Remove(bs)
}
func (i *Imp) DelAllData(collection string, bs bson.M) (int, error) {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	info, err := s.DB(i.DB).C(collection).RemoveAll(bs)
	if err != nil {
		return 0, err
	} else {
...
func (i *Imp) Aggregate(collection string, bs []bson.M, value any) error {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).Pipe(bs).All(value)
}
func (i *Imp) AggregateOne(collection string, bs []bson.M, value any) error {
	s := i.c.Ref()
	defer i.c.UnRef(s)
	return s.DB(i.DB).C(collection).Pipe(bs).One(value)
}
...
func (i *Imp) CreateIndex(collection string, unique bool, timeLimit bool, value []string) error {
	...
	return s.DB(i.DB).C(collection).EnsureIndex(index)
}
