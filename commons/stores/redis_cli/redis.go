...
// PExpire is the implementation of redis expire command.
func (s *Redis) PExpire(key string, duration time.Duration) error {
	return s.PExpireCtx(context.Background(), key, duration)
}

// PExpireCtx is the implementation of redis expire command.
func (s *Redis) PExpireCtx(ctx context.Context, key string, duration time.Duration) error {
	return s.brk.DoWithAcceptable(func() error {
		conn, err := getRedis(s)
		if err != nil {
			return err
		}
		return conn.PExpire(ctx, key, duration).Err()
	}, acceptable)
}
...
// SetEx is the implementation of redis setex command.
// by duration
func (s *Redis) SetEx(key, value string, duration time.Duration) error {
	return s.SetExCtx(context.Background(), key, value, duration)
}

func (s *Redis) SetExCtx(ctx context.Context, key, value string, duration time.Duration) error {
	return s.brk.DoWithAcceptable(func() error {
		conn, err := getRedis(s)
		if err != nil {
			return err
		}
		return conn.Set(ctx, key, value, duration).Err()
	}, acceptable)
}
...
// PTTL is the implementation of redis ttl command.
func (s *Redis) PTTL(key string) (time.Duration, error) {
	return s.PTTLCtx(context.Background(), key)
}

// PTTLCtx is the implementation of redis ttl command.
func (s *Redis) PTTLCtx(ctx context.Context, key string) (val time.Duration, err error) {
	err = s.brk.DoWithAcceptable(func() error {
		conn, err := getRedis(s)
		if err != nil {
			return err
		...
		duration, err := conn.PTTL(ctx, key).Result()
		if err != nil {
			return err
		}
...
// ZaddFloat is the implementation of redis zadd command.
func (s *Redis) ZaddArgs(key string, args red.ZAddArgs) (int64, error) {
	return s.ZaddArgsCtx(context.Background(), key, args)
}

// ZaddArgsCtx is the implementation of redis zadd command.
func (s *Redis) ZaddArgsCtx(ctx context.Context, key string, args red.ZAddArgs) (num int64, err error) {
	err = s.brk.DoWithAcceptable(func() error {
		conn, err := getRedis(s)
		if err != nil {
			return err
		...
		num, err = conn.ZAddArgs(ctx, key, args).Result()
		if err != nil {
			return err
		}
		return nil
...
// ZmScore is the implementation of redis ZmScore command.
func (s *Redis) ZmScore(key string, value ...string) ([]float64, error) {
	return s.ZmScoreCtx(context.Background(), key, value...)
}

// ZmScoreCtx is the implementation of redis ZmScore command.
func (s *Redis) ZmScoreCtx(ctx context.Context, key string, value ...string) (valList []float64, err error) {
	err = s.brk.DoWithAcceptable(func() error {
		conn, err := getRedis(s)
		if err != nil {
			return err
		...
		v, err := conn.ZMScore(ctx, key, value...).Result()
		if err != nil {
			return err
		}
...
func (s *Redis) ZrangeByScoreWithScores(key string, rangeBy *red.ZRangeBy) ([]Pair, error) {
	return s.ZrangeByScoreWithScoresCtx(context.Background(), key, rangeBy)
}

// ZrangebyscoreWithScoresCtx is the implementation of redis zrangebyscore command with scores.
func (s *Redis) ZrangeByScoreWithScoresCtx(ctx context.Context, key string, rangeBy *red.ZRangeBy) (
	val []Pair, err error) {
	err = s.brk.DoWithAcceptable(func() error {
		conn, err := getRedis(s)
		if err != nil {
		...
		v, err := conn.ZRangeByScoreWithScores(ctx, key, rangeBy).Result()
		if err != nil {
			return err
		}
...
// WithTLS customizes the given Redis with db selected.
