...
type (
	...
	Collection interface {
		...
		CreateIndexes(ctx context.Context, models []mongo.IndexModel, opts ...*mopt.CreateIndexesOptions) ([]string, error)
...
func (c *decoratedCollection) CreateIndexes(ctx context.Context, models []mongo.IndexModel, opts ...*mopt.CreateIndexesOptions) (res []string, err error) {
	ctx, span := startSpan(ctx, createIndexes)
	defer func() {
		endSpan(span, err)
	}()
	...
	err = c.brk.DoWithAcceptable(func() error {
		...
		res, err = c.Collection.Indexes().CreateMany(ctx, models, opts...)
		return err
	}, acceptable)
	return
}
