...
// Cast 异步投递消息
func (s *Server) Cast(uid uint32, req any) {
	id := MsgID(req)
	reqCtx := &ReqCtx{
		reqID: gid.ID(),
		id:    id,
...
// Call 发起同步调用，不带超时机制
func (s *Server) Call(uid uint32, req any) *AckCtx {
	return NewClient(0).Call(uid, s, req)
}

// CallT 发起同步调用，带超时机制
func (s *Server) CallT(uid uint32, req any, timeout time.Duration) *AckCtx {
	return NewClient(0).CallT(uid, s, req, timeout)
}
