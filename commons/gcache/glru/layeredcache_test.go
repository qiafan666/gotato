...
func Test_LayedCache_SetANewValue(t *testing.T) {
	...
	Equal(t, cache.Get("spice", "stop"), nil)
...
func Test_LayedCache_SetsMultipleValueWithinTheSameLayer(t *testing.T) {
	...
	Equal(t, cache.Get("spice", "flow").Value(), "value-a")
	Equal(t, cache.Get("spice", "must").Value(), "value-b")
	Equal(t, cache.Get("spice", "worm"), nil)

	Equal(t, cache.Get("leto", "sister").Value(), "ghanima")
	Equal(t, cache.Get("leto", "brother"), nil)
	Equal(t, cache.Get("baron", "friend"), nil)
	Equal(t, cache.ItemCount(), 3)
}

func Test_LayedCache_ReplaceDoesNothingIfKeyDoesNotExist(t *testing.T) {
	cache := newLayered[string]()
	Equal(t, cache.Replace("spice", "flow", "value-a"), false)
	Equal(t, cache.Get("spice", "flow"), nil)
}
...
func Test_LayedCache_ReplaceUpdatesTheValue(t *testing.T) {
	cache := newLayered[string]()
	cache.Set("spice", "flow", "value-a", time.Minute)
	Equal(t, cache.Replace("spice", "flow", "value-b"), true)
...
func Test_LayedCache_DeletesAPrefix(t *testing.T) {
	...
	Equal(t, cache.ItemCount(), 6)

	Equal(t, cache.DeletePrefix("spice", "9a"), 0)
	Equal(t, cache.ItemCount(), 6)

	Equal(t, cache.DeletePrefix("spice", "aa"), 3)
	Equal(t, cache.Get("spice", "aaa"), nil)
	Equal(t, cache.Get("spice", "aab"), nil)
	Equal(t, cache.Get("spice", "aac"), nil)
	Equal(t, cache.Get("spice", "ac").Value(), "4")
	Equal(t, cache.Get("spice", "z5").Value(), "7")
	Equal(t, cache.ItemCount(), 3)
}
...
func Test_LayedCache_DeletesAFunc(t *testing.T) {
	...
	Equal(t, cache.DeleteFunc("spice", func(key string, item *Item[int]) bool {
		return false
	}), 0)
	Equal(t, cache.ItemCount(), 6)
...
func Test_LayedCache_OnDeleteCallbackCalled(t *testing.T) {
	...
	Equal(t, atomic.LoadInt32(&onDeleteFnCalled), 1)
}
...
func Test_LayedCache_DeletesALayer(t *testing.T) {
	...
	Equal(t, cache.Get("spice", "flow"), nil)
	Equal(t, cache.Get("spice", "must"), nil)
	Equal(t, cache.Get("spice", "worm"), nil)
	Equal(t, cache.Get("leto", "sister").Value(), "ghanima")
}
...
func Test_LayeredCache_GCsTheOldestItems(t *testing.T) {
	...
	cache.SyncUpdates()
	cache.GC()
	Equal(t, cache.Get("xx", "a"), nil)
	Equal(t, cache.Get("xx", "b").Value(), 9001)
	Equal(t, cache.Get("8", "a"), nil)
	Equal(t, cache.Get("9", "a").Value(), 9)
	Equal(t, cache.Get("10", "a").Value(), 10)
}
...
func Test_LayeredCache_PromotedItemsDontGetPruned(t *testing.T) {
	...
	cache.GC()
	Equal(t, cache.Get("9", "a").Value(), 9)
	Equal(t, cache.Get("10", "a"), nil)
	Equal(t, cache.Get("11", "a").Value(), 11)
}
...
func Test_LayeredCache_RemovesOldestItemWhenFull(t *testing.T) {
	...
	Equal(t, cache.Get("0", "a"), nil)
	Equal(t, cache.Get("1", "a"), nil)
	Equal(t, cache.Get("2", "a"), nil)
	Equal(t, cache.Get("3", "a").Value(), 3)
	Equal(t, cache.Get("xx", "b").Value(), 9001)
	Equal(t, cache.GetDropped(), 4)
	Equal(t, cache.GetDropped(), 0)
	Equal(t, onDeleteFnCalled, true)
}
...
func Test_LayeredCache_ResizeOnTheFly(t *testing.T) {
	...
	cache.SetMaxSize(3)
	cache.SyncUpdates()
	Equal(t, cache.GetDropped(), 2)
	Equal(t, cache.Get("0", "a"), nil)
	Equal(t, cache.Get("1", "a"), nil)
	Equal(t, cache.Get("2", "a").Value(), 2)
	Equal(t, cache.Get("3", "a").Value(), 3)
	Equal(t, cache.Get("4", "a").Value(), 4)

	cache.Set("5", "a", 5, time.Minute)
	cache.SyncUpdates()
	Equal(t, cache.GetDropped(), 1)
	Equal(t, cache.Get("2", "a"), nil)
	Equal(t, cache.Get("3", "a").Value(), 3)
	Equal(t, cache.Get("4", "a").Value(), 4)
	Equal(t, cache.Get("5", "a").Value(), 5)
...
func Test_LayeredCache_RemovesOldestItemWhenFullBySizer(t *testing.T) {
	...
	cache.SyncUpdates()
	Equal(t, cache.Get("pri", "0"), nil)
	Equal(t, cache.Get("pri", "1"), nil)
	Equal(t, cache.Get("pri", "2"), nil)
	Equal(t, cache.Get("pri", "3"), nil)
	Equal(t, cache.Get("pri", "4").Value().id, 4)
}
...
func Test_LayeredCache_ReplaceChangesSize(t *testing.T) {
	...
	cache.SyncUpdates()
	Equal(t, cache.GetSize(), 4)

	cache.Replace("pri", "2", &SizedItem{1, 1})
	cache.SyncUpdates()
	Equal(t, cache.GetSize(), 3)
...
func Test_LayeredCache_EachFunc(t *testing.T) {
	...
	ListTest(t, forEachKeysLayered[int](cache, "1"), []string{})

	cache.Set("1", "a", 1, time.Minute)
	ListTest(t, forEachKeysLayered[int](cache, "1"), []string{"a"})

	cache.Set("1", "b", 2, time.Minute)
	cache.SyncUpdates()
	ListTest(t, forEachKeysLayered[int](cache, "1"), []string{"a", "b"})

	cache.Set("1", "c", 3, time.Minute)
	cache.SyncUpdates()
	ListTest(t, forEachKeysLayered[int](cache, "1"), []string{"a", "b", "c"})

	cache.Set("1", "d", 4, time.Minute)
	cache.SyncUpdates()
	ListTest(t, forEachKeysLayered[int](cache, "1"), []string{"b", "c", "d"})

	// iteration is non-deterministic, all we know for sure is "stop" should not be in there
	cache.Set("1", "stop", 5, time.Minute)
	cache.SyncUpdates()
	DoesNotContain(t, forEachKeysLayered[int](cache, "1"), "stop")
