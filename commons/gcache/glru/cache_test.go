...
func Test_Setnx(t *testing.T) {
	...
	Equal(t, cache.ItemCount(), 1)
	Equal(t, cache.Get("spice").Value(), "flow")

	// set if not exists
	cache.Delete("spice")
	cache.Setnx("spice", "worm", time.Minute)
	Equal(t, cache.Get("spice").Value(), "worm")
...
func Test_CacheDeletesAValue(t *testing.T) {
	...
	cache.Delete("spice")
	Equal(t, cache.Get("spice"), nil)
	Equal(t, cache.Get("worm").Value(), "sand")
	Equal(t, cache.ItemCount(), 1)
}
...
func Test_CacheDeletesAPrefix(t *testing.T) {
	...
	Equal(t, cache.ItemCount(), 5)

	Equal(t, cache.DeletePrefix("9a"), 0)
	Equal(t, cache.ItemCount(), 5)

	Equal(t, cache.DeletePrefix("aa"), 3)
	Equal(t, cache.Get("aaa"), nil)
	Equal(t, cache.Get("aab"), nil)
	Equal(t, cache.Get("aac"), nil)
	Equal(t, cache.Get("ac").Value(), "4")
	Equal(t, cache.Get("z5").Value(), "7")
	Equal(t, cache.ItemCount(), 2)
}
...
func Test_CacheDeletesAFunc(t *testing.T) {
	...
	Equal(t, cache.DeleteFunc(func(key string, item *Item[int]) bool {
		return false
	}), 0)
	Equal(t, cache.ItemCount(), 6)
...
func Test_CacheFetchesExpiredItems(t *testing.T) {
	...
	Equal(t, cache.Get("beef").Value(), "moo")

	out, _ := cache.Fetch("beef", time.Second, fn)
	Equal(t, out.Value(), "moo-moo")
}
...
func Test_CacheGCsTheOldestItems(t *testing.T) {
	...
	cache.SyncUpdates()
	cache.GC()
	Equal(t, cache.Get("9"), nil)
	Equal(t, cache.Get("10").Value(), 10)
	Equal(t, cache.ItemCount(), 490)
}
...
func Test_CachePromotedItemsDontGetPruned(t *testing.T) {
	...
	cache.SyncUpdates()
	cache.GC()
	Equal(t, cache.Get("9").Value(), 9)
	Equal(t, cache.Get("10"), nil)
	Equal(t, cache.Get("11").Value(), 11)
}
...
func Test_CacheTrackerDoesNotCleanupHeldInstance(t *testing.T) {
	...
	cache.SyncUpdates()
	cache.GC()
	Equal(t, cache.Get("0").Value(), 0)
	Equal(t, cache.Get("1").Value(), 1)
	item0.Release()
	item1.Release()
	cache.GC()
	Equal(t, cache.Get("0"), nil)
	Equal(t, cache.Get("1"), nil)
}
...
func Test_CacheRemovesOldestItemWhenFullBySizer(t *testing.T) {
	...
	Equal(t, cache.Get("0"), nil)
	Equal(t, cache.Get("1"), nil)
	Equal(t, cache.Get("2"), nil)
	Equal(t, cache.Get("3"), nil)
	Equal(t, cache.Get("4").Value().id, 4)
...
func Test_CacheResizeOnTheFly(t *testing.T) {
	...
	Equal(t, cache.Get("1"), nil)
	Equal(t, cache.Get("2").Value(), 2)
	Equal(t, cache.Get("3").Value(), 3)
	Equal(t, cache.Get("4").Value(), 4)
	...
	Equal(t, cache.Get("4").Value(), 4)
	Equal(t, cache.Get("5").Value(), 5)
	Equal(t, cache.Get("6").Value(), 6)
}

func Test_CacheForEachFunc(t *testing.T) {
	cache := New(Configure[int]().MaxSize(3).ItemsToPrune(1))
	ListTest(t, forEachKeys[int](cache), []string{})

	cache.Set("1", 1, time.Minute)
	ListTest(t, forEachKeys(cache), []string{"1"})

	cache.Set("2", 2, time.Minute)
	cache.SyncUpdates()
	ListTest(t, forEachKeys(cache), []string{"1", "2"})

	cache.Set("3", 3, time.Minute)
	cache.SyncUpdates()
	ListTest(t, forEachKeys(cache), []string{"1", "2", "3"})

	cache.Set("4", 4, time.Minute)
	cache.SyncUpdates()
	ListTest(t, forEachKeys(cache), []string{"2", "3", "4"})

	cache.Set("stop", 5, time.Minute)
	cache.SyncUpdates()
	DoesNotContain(t, forEachKeys(cache), "stop")
