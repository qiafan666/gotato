...
type TimerMgr struct {
	...
	dispatcher     gtimer.ITimerAPI
}

// NewTimerMgr 通常结合 skeleton module使用
func NewTimerMgr(dispatcher gtimer.ITimerAPI, dbname, collName string) *TimerMgr {
	return &TimerMgr{
		collName:       collName,
		dbName:         dbname,
...
func NewTimerMgrNoDB(dispatcher gtimer.ITimerAPI) *TimerMgr {
	return &TimerMgr{
		Timers:         nil,
		timersUnNeedDB: make(map[int64]*Timer),
		handlers:       make(map[int32]*TimerMeta),
...
func (tm *TimerMgr) initAfterDB() {
	// 恢复timer
	ops := make([]*gtimer.NewOp, 0, len(tm.Timers))
	for _, t := range tm.Timers {
		ops = append(ops, &gtimer.NewOp{
			Typ:   t.TimerType,
			ID:    t.TimerID,
			EndTs: t.EndTs,
			Cb:    tm.timerCommonCb,
