...
func (s *ZkClient) CreateTempNode(rpcRegisterName, addr string) (node string, err error) {
	...
	if err != nil {
		return "", gerr.WrapMsg(err, "CreateProtectedEphemeralSequential failed", "path", s.getPath(rpcRegisterName)+"/"+addr+"_")
	}
	return node, nil
}
...
func (s *ZkClient) Register(rpcRegisterName, host string, port int, opts ...grpc.DialOption) error {
	...
	if err != nil {
		return gerr.WrapMsg(err, "grpc dial error", "addr", addr)
	}
...
func (s *ZkClient) UnRegister() error {
	...
	if err != nil {
		return gerr.WrapMsg(err, "delete node error", "node", s.node)
	}
