...
func CreateProducer(config ProducerConfig) (*Producer, error) {
	...
	if err != nil {
		glog.Slog.ErrorF(nil, "rabbitmq connect error: %s", err.Error())
		return nil, err
	}
...
func (p *Producer) Send(data []byte) bool {
	...
	_, err = ch.QueueDeclare(
		...
		p.config.Durable,   //队列是否持久化，false模式数据全部处于内存，true会保存在erlang自带数据库，但是影响速度
	...
	err = ch.PublishWithContext(
		p.ctx,
		"",                 // workqueue 模式设置为空字符串，表示使用默认交换机
		p.config.QueueName, // 注意：简单模式 key 表示队列名称
		false,
		false,
		amqp.Publishing{
