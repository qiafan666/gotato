...
func SetProdMsgDelayParams(rabbitMqDelayExchangeName string, enableMsgDelayPlugin bool) OptionsProd {
	return OptionFunc(func(p *Producer) {
		...
		p.config.ExchangeName = rabbitMqDelayExchangeName
...
func SetConsMsgDelayParams(rabbitMqDelayExchangeName string, enableDelayMsgPlugin bool) OptionsConsumer {
	return OptionsConsumerFunc(func(c *Consumer) {
		...
		c.config.ExchangeName = rabbitMqDelayExchangeName
		// 延迟消息队列，交换机、消息全部设置为持久
